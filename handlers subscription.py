...
async def subscribe_menu(message: Message) -> None:
    ...
    if db.is_pro(user):
        ...
        await message.answer(f"У тебя уже активна подписка до {until}. Можно продлить заранее:")
    else:
    ...
    await message.answer("Тарифы:", reply_markup=subscription_plans_kb(PLANS))
