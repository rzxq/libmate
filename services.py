...
1) Google Books API — бесплатный, без ключа для базового поиска, хорошо
   ...
   langRestrict=ru). Используем для поиска обложки/описания/автора, а также
   для РЕЙТИНГА (averageRating/ratingsCount), ДОСТУПНОСТИ в электронном виде
   (accessInfo.epub/pdf.isAvailable) и ВОЗМОЖНОСТИ КУПИТЬ (saleInfo).
...
class BookInfo:
    ...
    # Рейтинг по 5-звёздочной шкале и количество оценок (данные Google Books).
    average_rating: Optional[float] = None
    ratings_count: Optional[int] = None
    # Доступность в электронном виде (epub/pdf) по данным Google Books.
    is_ebook: Optional[bool] = None
    # Можно ли купить через Google Books и ссылка на покупку.
...
def format_availability(info) -> str:
    """Доступность книги в электронном виде. Принимает BookInfo или db.Book —
    у обоих есть атрибут is_ebook."""
    is_ebook = getattr(info, "is_ebook", None)
    if is_ebook is True:
        return "📱 Доступность: есть электронная версия (epub/pdf)"
    if is_ebook is False:
        return "📱 Доступность: электронную версию не нашёл, вероятно только бумага"
    return "📱 Доступность: нет данных"


def format_purchase(info) -> str:
    """Можно ли купить книгу. Принимает BookInfo или db.Book."""
    ...
    if for_sale is False:
        return "🛒 Покупка: не продаётся через Google Books (поищи на других площадках)"
...
def _parse_volume(item: dict) -> BookInfo:
    ...
    epub_avail = access.get("epub", {}).get("isAvailable")
    pdf_avail = access.get("pdf", {}).get("isAvailable")
    if epub_avail is None and pdf_avail is None:
        is_ebook = None
    else:
        is_ebook = bool(epub_avail or pdf_avail)

    saleability = sale.get("saleability")
    for_sale = (saleability == "FOR_SALE") if saleability is not None else None
    buy_link = sale.get("buyLink") if for_sale else None
...
async def ai_find_book(query: str) -> Optional[BookInfo]:
    ...
    Рейтинг/доступность/покупку через этот путь узнать нельзя (у модели нет
    доступа к структурированным данным продавца), поэтому эти поля остаются
    пустыми — в карточке книги будет честно написано "нет данных".
    """
