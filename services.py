...
async def _google_books_request(query: str, lang_restrict: Optional[str]) -> list[dict]:
    ...
    try:
        ...
        logger.info("Google Books: запрос %r (lang=%s) → %d результатов", query, lang_restrict, len(items))
        return items
    except httpx.HTTPStatusError as e:
        logger.warning(
            "Google Books HTTP %s для запроса %r: %s | тело ответа: %s",
            e.response.status_code, query, e, e.response.text[:500],
        )
        return []
